...
const classThreshold = 0.8;

// wait until opencv.js initialized
cv["onRuntimeInitialized"] = async () => {
  // create session
  const [yolov5, nms, mask, segnet] = await Promise.all([
    ort.InferenceSession.create(`model/best-yolov5.onnx`),
    ort.InferenceSession.create(`model/nms-yolov5.onnx`),
    ort.InferenceSession.create("model/mask-yolov5-seg.onnx"),
    ort.InferenceSession.create("model/best-seg-640-yolov5.onnx"),
  ]);
  ...
  mySession = setSession({ net: yolov5, nms: nms, mask: mask, segnet: segnet });
};
...
const [input, xRatio, yRatio] = preprocessing(img1, modelWidth, modelHeight);
...
const classThreshold = 0.2;
  ...
  const { output0, output1 } =  await session.segnet.run({ images: tensor1 });
  console.log(output1);
//console.log(output0);
//console.log(output1);
//var output3 = output0;
...
console.log(data.slice(4 + numClass));
      const mask = new ort.Tensor(
        ...
        new Float32Array([
          ...
          ...data.slice(5 + numClass), // mask data // for yolov5 its 5 + 
        ])
      ); // mask input
      const maskConfig = new ort.Tensor(
        "float32",
	  ...
	  /*
const colorFrequencyMap = countColors(new ImageData(new Uint8ClampedArray(mask_mat.data), overlay.cols,overlay.rows));

//console.log("Color Frequency Map:");
//console.log(colorFrequencyMap);
//	  //console.log(label);

const count = countNonZeroPixels(new ImageData(new Uint8ClampedArray(mask_mat.data), overlay.cols,overlay.rows));
//console.log(label);
if (label == 1 )
	berry_pixels = berry_pixels+count;
else if (label == 0 )
	bruise_pixels = bruise_pixels+count;
//console.log(`Number of pixels with non-zero RGB values: ${count}`);
*/
      mask_mat.delete(); // delete unused Mat
    }
  }
...
paragraph.textContent = `Bruise Pixels: ${bruise_pixels}, Berry Pixels: ${berry_pixels}, Ratio: ${bruise_ratio1.toFixed(1)}`;
...
// Set the text content of the paragraph
paragraph1.textContent = `Mean bruise ratio ${mean_ratio.toFixed(1)}`;
// Append the paragraph to the container
